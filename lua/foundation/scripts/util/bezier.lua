...
function Bezier.calc_point(t, p1, c1, c2, p2)
	local t_inv = 1 - t
...
function Bezier.calc_tangent(t, p1, c1, c2, p2)
	local res = Vector3.normalize(-3 * (p1 * (t - 1)^2 + c1 * (-3 * t^2 + 4 * t - 1) + t * (3 * c2 * t - 2 * c2 - p2 * t)))

	return res
end

function Bezier.draw(segments, script_drawer, tangent_scale, color, p1, c1, c2, p2)
...
function Bezier.length(segments, p1, c1, c2, p2)
	local length = 0
	local last_point = p1
...
function Bezier.next_index(points, index)
...
function Bezier.spline_points(points, index)
