...
function PerfhudManager:init()
	self._active_huds = {}
	self._accumulated_index = nil
end

function PerfhudManager:update(dt, t)
...
function PerfhudManager:_update_peer_index(dt, t)
...
function PerfhudManager:_parse_keystrokes(strokes)
...
function PerfhudManager:_set_peer_index(accumulated_index)
	Application.console_command("perfhud", "network_peer", accumulated_index)
end

function PerfhudManager:_toggle_hud(hud_name, shift_held)
...
function PerfhudManager:_close_all_huds()
	for hud_name, _ in pairs(self._active_huds) do
		self:_close_hud(hud_name)
	end
end

function PerfhudManager:_open_hud(hud_name)
...
function PerfhudManager:_close_hud(hud_name)
	self._active_huds[hud_name] = nil

	Application.console_command("perfhud", hud_name)
end

function PerfhudManager:destroy(dt, t)
	self:_close_all_huds()
end
